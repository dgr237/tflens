...
- **Tracked-attribute records.** Still emit `expression_text` only — the underlying `*Expr` is private to `pkg/analysis.TrackedAttribute`. Promoting them to the full `{text, value?, ast?}` shape is a small follow-up.
- **`lifecycle`** is deliberately not in either `blocks` or `dynamic_blocks` because its meta-arg attributes are already projected into dedicated parent-resource fields (`prevent_destroy`, `ignore_changes`, etc.) — surfacing it twice would just be noise.
