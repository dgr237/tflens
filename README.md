...
- **No provider schemas embedded — but two opt-in paths.** We do not bundle AWS/GCP/Azure/etc. provider schemas in the binary. Running `terraform validate` in addition to this tool catches type / required-vs-optional / deprecation issues by default. Two flags opt into schema-aware behaviour: `--provider-schema path/to/schema.json` accepts user-supplied `terraform providers schema -json` output for attribute-name validation and richer cross-module type inference; `--immutable-table-override path/to/extra.json` extends the embedded force-new attribute table (`pkg/forcenew`, ~1000 upjet-supported AWS resources, ~4632 attribute paths extracted from Crossplane's runtime IR) that drives automatic Breaking classification on destroy+recreate changes. The two flags are complementary — the Terraform schema-json format doesn't carry force-new metadata, so each flag covers a distinct concern. Resources outside the embedded force-new table also work with [`# tflens:track` markers](docs/commands/tracked-attributes.md) as a source-only alternative.
