...
ResourceGraphDefinition (RGD) where each resource template is a
Crossplane managed resource from the Upbound provider-aws catalogue
(eks.aws.upbound.io, iam.aws.upbound.io, ec2.aws.upbound.io, …).

This is the third POC in the series. The other two are:

  - docs/export-to-kro-rgd/        kro RGD + ACK CRDs
  - docs/export-to-crossplane/     Crossplane Composition + XRD + Upbound MRs

This one is the hybrid: kro's CEL substitution + lifecycle for the
RGD shape, Crossplane's provider-aws CRDs for the actual managed
resources. Useful when:

  - You want kro's CEL flexibility (string concat, json.marshal,
    .map() iteration) over Crossplane's narrower transforms vocabulary
  - You want the Crossplane provider-aws CRD catalogue (much wider
    AWS coverage than ACK's at the time of writing)
  - You don't want to commit to full Crossplane Compositions

Most of the generator carries over from the kro+ACK POC unchanged.
The deltas are:

  1. CROSSPLANE_MAPPING uses Upbound apiVersions (eks.aws.upbound.io
     instead of eks.services.k8s.aws); attribute renames follow
     Upbound's Terraform-style camelCase (roleArn, not roleARN).

  2. CROSSPLANE_ARN_PATH = "status.atProvider.arn" — every Upbound
     MR exposes its AWS ARN under status.atProvider, mirroring the
     forProvider/atProvider pattern Crossplane uses to separate
     user-controlled from provider-controlled fields.

  3. Resource attributes + nested blocks live under
     spec.forProvider.<X> rather than spec.<X> directly.
...
The translation model:
    ...
    resource "T" "N" { ... }   →   spec.resources[].id = N
                                   .template = Upbound MR; fields under
                                   spec.forProvider; renamed per Upbound's
                                   Terraform-style camelCase convention
    var.X                      →   ${schema.spec.X}
    resource_type.foo.arn      →   ${resources.foo.status.atProvider.arn}
    format("%s-y", var.x)      →   "${schema.spec.x + \"-y\"}"
    snake_case_attr            →   camelCaseAttr (Upbound convention)
    nested_block { ... }       →   nested_block: { ... } (recursive)
    dynamic "ingress" { ... }  →   CEL .map() over the for_each source
...
# ----------------------------------------------------------------------
# Mapping table: Terraform resource type → Upbound provider-aws CRD shape.
# Production converters would pull this from Upbound's CRD catalogue
# (https://marketplace.upbound.io/providers/upbound/provider-aws). Upbound
# auto-generates from the Terraform AWS provider, so attribute names are
# the camelCased Terraform names rather than ACK's AWS-API spellings —
# `roleArn` not `roleARN`, `assumeRolePolicy` not `assumeRolePolicyDocument`.
...
# Upbound convention: every provider-aws MR exposes its AWS ARN under
# status.atProvider.arn (mirroring the forProvider/atProvider split
# Crossplane uses to separate user-controlled vs provider-controlled
# fields). Cross-resource ARN refs in Terraform (resource.foo.arn)
# translate to that path.
CROSSPLANE_ARN_PATH = "status.atProvider.arn"
...
def traversal_to_cel(steps):
    ...
    if root in CROSSPLANE_MAPPING and rest and rest[0]["step"] == "attr":
        res_name = rest[0]["name"]
        attr_chain = [s["name"] for s in rest[1:] if s["step"] == "attr"]
        # The .arn shortcut: every Upbound MR exposes its ARN at the
        # standard atProvider path, so resource_type.foo.arn →
        # CROSSPLANE_ARN_PATH.
        if attr_chain == ["arn"]:
            return f"resources.{res_name}.{CROSSPLANE_ARN_PATH}"
        # Other attributes live under status.atProvider.<X> (mirroring
        # the forProvider/atProvider Crossplane convention).
        path = ".".join(attr_chain)
        return f"resources.{res_name}.status.atProvider.{path}"
...
def emit_resource(res):
    ...
    Critical delta from the kro+ACK POC: every Crossplane MR puts its
    user-controlled fields under spec.forProvider, NOT directly under
    spec. The status fields land under status.atProvider (status.X
    where X comes from the AWS API). The kro emitter doesn't care
    about either side — we just nest the spec one level deeper here."""
    tf_type = res["type"]
    if tf_type not in CROSSPLANE_MAPPING:
        return {
            "id": res["name"],
            "_unsupported": (
                f"No Crossplane provider-aws mapping for Terraform resource "
                f"type {tf_type!r}. Add it to CROSSPLANE_MAPPING."),
        }
    mapping = CROSSPLANE_MAPPING[tf_type]

    # for_provider holds everything the kro+ACK POC put directly under
    # spec — attributes + nested blocks + dynamic-block .map() results.
    for_provider = {}
    ...
    for name, expr in (res.get("attributes") or {}).items():
        ...
        for_provider[renamed] = expr_to_emit(expr)

    block_renames = mapping.get("block_renames", {})
    for name, instances in (res.get("blocks") or {}).items():
        renamed = block_renames.get(name, to_camel(name))
        if len(instances) == 1:
            for_provider[renamed] = emit_block(instances[0], mapping)
        else:
            for_provider[renamed] = [emit_block(b, mapping) for b in instances]

    for name, instances in (res.get("dynamic_blocks") or {}).items():
        renamed = block_renames.get(name, to_camel(name))
        rendered = [emit_dynamic(d, name, mapping) for d in instances]
        for_provider[renamed] = rendered if len(rendered) > 1 else rendered[0]
    ...
    return {
        ...
        "template": {
            ...
            "kind": mapping["kind"],
            "metadata": {"name": for_provider.get("name", res["name"])},
